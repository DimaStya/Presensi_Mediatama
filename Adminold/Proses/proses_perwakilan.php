...
if ($aksi == "simpan"){
	...
	if (empty($kode_perwakilan)){
		$sql = mysqli_query($con, "SELECT max(kode_perwakilan) FROM tbl_perwakilan WHERE kode_area='".$_POST['kode_area']."'");
		$data = mysqli_fetch_array($sql);
		if($data['max(kode_perwakilan)']== null){
			$perwakilan_kode = $_POST['kode_area'].".1";
		}else{
			$kode = explode('.', $data['max(kode_perwakilan)']);
			$tambah = $kode[2]+1;
		...
		if ($_POST['status']=="Aktif"){
			$upuser = mysqli_query($con, "UPDATE tbl_user SET pass='".$pass."',username ='".strtolower($_POST['email'])."' WHERE kode='".$_POST['kode_perwakilan']."'");
			if($upuser){
